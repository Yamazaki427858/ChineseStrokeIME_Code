...
連續點選多個 Emoji 時，送出策略改為**佇列批次合併**：

- 多個待插入字元先進入佇列，再合併為較少的輸出操作一次送出
- **提升單輪吞吐**：連按表情時整體插入速度更快
- **減少 `SendInput` 呼叫次數**：降低與目標應用程式互動的開銷，減輕部分程式在連續輸入時的卡頓

從 Emoji 選單連續選取多個符號時，體感會比舊版更跟手。
